...
<div class="flex h-full flex-col overflow-hidden">
  ...
  <div class="flex flex-1 overflow-hidden">
        ...
        {#snippet actions()}
          <div class="flex flex-row items-center">
            ...
            <button
              ...
              title="查看{docKey.replace('Diagram', '')}图表文档">
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      <Card title="图表" isClosable={false}>
        {#snippet actions()}
          <div class="flex flex-row items-center gap-2">
            ...
            <a
              ...
              title="在新页面查看图表"
              ><i class="fas fa-external-link-alt"></i>全屏</a>
            {#if env.isEnabledMermaidChartLinks}
              <a
                href={`${MCBaseURL}/app/plugin/save?state=${$stateStore.serialized}`}
                target="_blank"
                class="btn btn-secondary btn-xs gap-1 bg-[#FF3570]"
                title="保存图表到 Mermaid Chart"
                ><img src="./mermaidchart-logo.svg" class="h-5 w-5" alt="Mermaid chart logo" />保存
                到 Mermaid Chart</a>
            {/if}
          </div>
        {/snippet}
